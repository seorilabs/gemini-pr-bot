...
- Allows selected public repositories with `PUBLIC_REPOSITORY_ALLOWLIST`: `seorilabs/.github`, `seorilabs/platform`, and `seorilabs/seorilabs-backoffice` for central governance PRs, plus `seorilabs/seorilabs-official` for public product and privacy documentation. Automatic review on an allowlisted public repository still requires a same-repository PR whose author has current `write`, `maintain`, or `admin` repository permission; a trusted maintainer must explicitly request review for external forks.
...
PUBLIC_REPOSITORY_ALLOWLIST=seorilabs/.github,seorilabs/platform,seorilabs/seorilabs-backoffice,seorilabs/seorilabs-official
