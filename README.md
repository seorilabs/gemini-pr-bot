...
- Maps only host-recognized checklist items or bullets under acceptance/requirements/definition-of-done/behavior headings (`동작`, `기대 동작` 포함) to a bounded host-extracted menu of exact current-HEAD test or source evidence; AC IDs, explicit identifiers, and literal overlap rank relevant lines first so late assertions in large smoke files are not starved by the prompt budget. Unchanged tests count when their test body and assertion are present in context. The model selects evidence but cannot invent its identity. Validation-result sections such as `검증` are not acceptance criteria.
...
- Test evidence must come from a registered, active current-HEAD test with a real non-vacuous assertion. Composite criteria may select one primary plus up to three supporting lines from the same executable test; setup calls are valid only when paired with assertions, and the host validates every explicitly named output. For example, `sim=28800, rollover=0` requires assertions for both outputs and a missing one is named in the Contributor response. A model-provided line number is rebound only when its exact assertion quote occurs uniquely in the current HEAD; a trailing Godot multiline continuation marker may be omitted, but fabricated or ambiguous quotes remain rejected. Commented code, skipped/disabled tests or suites, ordinary helper functions, comparison-only expressions, and assertion-shaped strings are rejected by the host.
...
- Keeps a host-grounded acceptance PASS sticky for the same HEAD and prompt contract: later model variance may replace it with better grounded evidence but cannot regress it to `unknown`. A new commit still invalidates the old result and is reviewed normally.
