...
- Test evidence must come from a registered, active current-HEAD test with a real non-vacuous assertion. Composite criteria may select one primary plus up to three supporting lines from the same executable test; setup calls are valid only when paired with assertions, and the host validates the combined contract. A model-provided line number is rebound only when its exact assertion quote occurs uniquely in the current HEAD; a trailing Godot multiline continuation marker may be omitted, but fabricated or ambiguous quotes remain rejected. Commented code, skipped/disabled tests or suites, ordinary helper functions, comparison-only expressions, and assertion-shaped strings are rejected by the host.
