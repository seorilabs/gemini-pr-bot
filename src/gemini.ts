...
export class GeminiClient {
  ...
  private structuredReviewPrompt(prompt: string): string {
    return [
      ...
      "- The convergence rule applies ONLY to high/critical (merge-blocking) severity, never to medium/low: when prior high/critical findings are addressed, do NOT manufacture a new high/critical to keep blocking. Only raise a new high/critical for a genuinely new, diff-evidenced product defect.",
