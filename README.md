...
- Keeps a host-grounded acceptance PASS sticky for the same HEAD across compatible evidence-cache schemas: later model variance or a prompt revision may replace it with better grounded evidence but cannot regress it to `unknown`. The Host revalidates every carried exact quote against the current evidence index. A new commit still invalidates the old result and is reviewed normally.
...
By default, `DEEP_REPO_CONTEXT_MODE=auto` shallow-clones the PR head only for code, workflow, action, and config changes that benefit from surrounding repository context. The clone produces two independent outputs: a bounded set of related text files for the model prompt, and a Host-only current-HEAD test evidence index used to extract and verify exact assertions. Prompt truncation does not remove tests from the Host index. `test_inventory_complete` describes the Host scan, not whether every test body fit in the prompt. The temporary checkout is then deleted from `/tmp`.
