...
export class GeminiClient {
  ...
  private structuredReviewPrompt(prompt: string): string {
    return [
      ...
      "- Never report that code, tests, config, validation, or migration is 'missing', 'not included', 'not visible', 'not confirmed', or 'unclear from the PR'. If you cannot see it, you cannot claim it is wrong — stay silent.",
      ...
      "- Converge: when prior blocking findings are addressed, do NOT manufacture a new high/critical to keep blocking. Only raise a new high/critical for a genuinely new, diff-evidenced product defect. If none exists, return an empty array and let the PR pass.",
      "- Before emitting any finding, drop it unless you can quote the specific diff line(s) that prove it. If the basis is 'not shown' or 'inferred tool behavior', do not emit it.",
      "",
