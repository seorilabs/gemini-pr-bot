...
- Maps only host-recognized checklist items or bullets under acceptance/requirements/definition-of-done/behavior headings (`동작`, `기대 동작` 포함) to a bounded host-extracted menu of exact current-HEAD test or source evidence; unchanged tests count when their test body and assertion are present in context. The model selects evidence but cannot invent its identity. Validation-result sections such as `검증` are not acceptance criteria.
